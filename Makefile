...
install :; forge install cyfrin/foundry-devops@0.2.2 --no-commit && forge install smartcontractkit/chainlink-brownie-contracts@1.2.0 --no-commit && forge install foundry-rs/forge-std@v1.9.2 --no-commit
...
SEPOLIA_NETWORK_ARGS := --rpc-url $(RPC_URL_SEPOLIA) --account $(ACCOUNT) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
...
deploy-zk-sepolia:
	@forge create src/FundMe.sol:FundMe --rpc-url ${RPC_URL_ZKSYNC_SEPOLIA} --account default --constructor-args 0xfEefF7c3fB57d18C5C6Cdd71e45D2D0b4F9377bF --legacy --zksync
